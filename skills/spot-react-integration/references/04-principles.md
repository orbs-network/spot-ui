...
3. **Use DEX Components As-Is** — Don't modify DEX components. If one doesn't exist (e.g. Select), create a new one using DEX styles. Never use generic web components.
...
10. **Format Display Values** — Use DEX's formatting function or `useFormatNumber` from spot-react.
11. **No Extra Packages** — Only install `@orbs-network/spot-react`. Use what the DEX already has.
12. **Don't Recreate Error Boundary** — Use the DEX's existing error boundary.
...
16. **Input Reset in onClose** — Reset the typed input amount when `isSuccess` is true inside the modal's `onClose` callback, after calling `resetState()` (see [02-provider.md](02-provider.md)).
...
- [ ] Input reset handled in modal `onClose` when `isSuccess`, after `resetState()`
...
- [ ] Submit modal built using `useSubmitOrderPanel()` data (srcToken, dstToken, srcAmount, dstAmount, status, step, etc.)
- [ ] Order history built using `useOrderHistoryPanel()` and `useDerivedHistoryOrder()`
