...
3. Read [references/03-panels.md](references/03-panels.md) for focused form, execution, configuration, and order-history hooks, plus required submit-modal display rules, allowance loading, all disclaimer texts and their display conditions, default disclaimer acceptance, and DEX number formatting. Register the disclaimer copy in the host's i18n resources and use the English defaults whenever translations are missing or empty.
...
7. Use [assets/spot-form-skeleton.tsx](assets/spot-form-skeleton.tsx) as a starting template.
8. Before finalizing, verify current public exports/types in `@orbs-network/spot-react` and `@orbs-network/swap-ui`; do not assume internal hooks or `dist/*` paths exist.
