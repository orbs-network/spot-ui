...
export const OrderFormProvider = ({ children }: { children: ReactNode }) => {
  ...
  const limitPriceUi = useSpotStore((store) => store.state.limitPriceUi);
  const triggerPriceUi = useSpotStore((store) => store.state.triggerPriceUi);
  ...
  const isPriceInverted = useSpotStore((store) => store.state.isPriceInverted);
  ...
  const frozenForm = useSpotStore((store) => {
    ...
    return execution.phase !== ExecutionPhase.IDLE ? execution.form : undefined;
  });
