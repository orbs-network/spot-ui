...
import type { Address, Hex, Module } from "../shared/types";

const Version = 0.7;
const BI_ENDPOINT = `https://bi.orbs.network/putes/twap-ui-${Version}`;

// Compatibility default for callers of the deprecated singleton.
let UI_VERSION = "unknown";

/** @deprecated Pass uiVersion to createClient instead. */
...
export class Analytics {
  ...
  private configDetails: Partial<Data> = {};
  private moduleImportKey = "";
  private data: Data = {
    _id: generateId(),
  };

  private async updateAndSend(
    values = {} as Partial<Data>,
    noTimeout = false,
    callback?: () => void,
  ) {
    try {
  ...
  init(partner: Partners, permitData: RePermitData) {
    ...
    const moduleImportData: Data = {
      ...
      uiVersion: this.options.uiVersion ?? "unknown",
      origin:
...
/** Event-only contract; mutable payloads and timers remain internal. */
export type SpotAnalytics = Pick<
  Analytics,
