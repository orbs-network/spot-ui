...
import { THE_GRAPH_ORDERS_API } from "./deployments";
import { getExchanges } from "./exchanges";
import { buildV1Order, type RawStatus } from "./normalize";
import { FillV1, GetV1OrdersFilters, OrderV1 } from "./types";

const normalizeSubgraphList = <T>(
  list?: T[],
...
export const getOrders = async ({
  ...
  try {
    ...
    for (const status of statuses) {
      ...
      const key = JSON.stringify([
        String(status.twapId),
        status.twapAddress.toLowerCase(),
      ]);
