...
import { MAX_UINT_256 } from "../../shared/evm-constants";
...
const getOrderType = (order: OrderV2) => {
  ...
  const isTakeProfit = isLegacyTakeProfit || BN(triggerUpper || 0).gt(0);
  const isStopLoss =
    (!isLegacyTakeProfit && BN(stop || 0).gt(0)) || BN(triggerLower || 0).gt(0);
...
const getStatus = (order: OrderV2, progress: number) => {
  ...
  if (status === "completed" || progress === 100) return OrderStatus.Completed;
  if (["pending", "eligible"].includes(status)) return OrderStatus.Open;
  if (
